...
int main(void) {
...

	{
		...
		cs540::Function<int()> ret_three_hundred_lambda(
				ret_three_hundred_lambda_func);

		//Test value construction with a functor
		cs540::Function<int()> ret_two_hundred_functor(
...

		{
			...
			assert(
					(!ret_one_hundred.operator bool())
							== (ret_one_hundred == nullptr));
			assert(
					(!ret_one_hundred.operator bool())
							== (nullptr == ret_one_hundred));
